...
- B2 metadata-only projection boundary is complete. B2 resolves the owner-accepted 41-entry source working set into 36 projected B2 items with 5 merge groups and stable tracks: `monitoring_track` and `risk_awareness_track`. This grouping remains private/read-only audit/projection metadata in `index.html`, separate from `buildSeasonalSnapshot`, not persisted, not exposed globally, not a new user data model, and not a registry. Runtime Slice 8 Step 1 is complete: Plant detail now consumes B2 metadata through an explicit private S8 read path and renders calm read-only monitoring/risk preview sections only for matching plant species. B2 still does not add `monitoring_programs[]`, `stage_vocabulary[]`, `awareness_definitions[]`, `target_registry[]`, `symptom_registry[]`, observation writes, Pregled/Kalendar monitoring UI, or broader Slice 8 behavior.
...
- S8 Step 2 note Observation shape is now documented: the next allowed runtime implementation, after owner approval, is Plant detail `kind = "note"` capture with `payload.text` plus Dnevnik evidence under `Opažanja`. Runtime implementation is still not done. Hard guardrails remain: no Pregled/Kalendar monitoring or risk UI, no structured trap/scouting/symptom/stage capture, no program attachment, no registries/vocabularies, no observation correction, no `Bez zapisa` / `Zadnji zapis`, no diagnosis/treatment, and no snapshot changes.
- Monitoring/Praćenje remains product-critical and long-term belongs in Pregled and Kalendar, but Pregled/Kalendar monitoring integration remains deferred until timing/display semantics are owner-approved. Structured observation capture, Dnevnik monitoring evidence, and stage confirmation are later Slice 8 steps; stage confirmation depends on owner-approved minimal stage vocabulary. Next work is the next owner-approved S8 implementation prompt or S8 planning.
...
- Runtime Slice 8 Step 1 is complete; later S8 work must remain owner-approved and must preserve the Plant detail-only first-step boundaries unless the owner explicitly opens the next step
