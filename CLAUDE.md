...
- B2 remains required before Slice 8. B2 currently resolves the owner-accepted 41-entry source working set into 36 projected B2 items: 30 štetnici/bolesti monitoring items and 6 promatranje/rizik/stanje items. This is the current resolved projection from the source map, not immutable final catalog truth. B2 uses `monitoring_programs[]` plus explicit source-map projection grouping by stable ids: `monitoring_track` and `risk_awareness_track`. This grouping is audit/projection metadata, not a new user data model and not a registry. B2 does not add `awareness_definitions[]`, `target_registry[]`, or `symptom_registry[]`. B2 must seed a minimal generic MVP `stage_vocabulary[]` with `dormant`, `bud_swell`, `bloom`, `fruit_set`, `ripening`, and `leaf_drop` only if that does not invent species-specific phenology, BBCH, timing, ordering, or plan effects; otherwise Slice 8 stage confirmation writes stay deferred/restricted until vocabulary exists. B2 also feeds Kalendar Praćenje, Pregled Praćenje, Plant detail monitoring/risk surfaces, and observation capture / observation rows; Slice 7 must not rely on monitoring or awareness content until B2 lands
...
- Rich agronomic instruction/details for seasonal actions are deferred to Runtime Slice 7 Detalj sezonske radnje; Slice 7 may now plan against canonical `action_window_definitions[].notes` from B1/B1.1 but must still not implement Monitoring/Praćenje. Slice 7 is not started and must not begin before owner approval and a fresh consistency check
...
- implementation was forbidden through S11 documentation; Slices 0, 1, 2, 3, 4, 5, and 6 plus the Pre-Slice-7 Action Window Notes Projection prerequisite (B1 and B1.1) were then owner-approved and committed
