...
- B2 metadata-only projection boundary is complete. B2 resolves the owner-accepted 41-entry source working set into 36 projected B2 items with 5 merge groups and stable tracks: `monitoring_track` and `risk_awareness_track`. This grouping is private/read-only audit/projection metadata in `index.html`, separate from `buildSeasonalSnapshot`, not persisted, not exposed globally, not rendered, not a new user data model, and not a registry. B2 does not add `monitoring_programs[]`, `stage_vocabulary[]`, `awareness_definitions[]`, `target_registry[]`, `symptom_registry[]`, observation writes, monitoring UI, risk-awareness UI, or Slice 8 behavior. Runtime Slice 8 has not started; the approved first S8 implementation scope is Plant detail read-only B2 monitoring/risk preview only through an explicit S8 read path
- First S8 guardrails: do not touch Pregled or Kalendar; do not add observation writes, Dnevnik observation rows, routes, tap-through detail, or CTAs such as `Dodaj opažanje`; do not show `Bez zapisa` until observation capture exists; do not change `buildSeasonalSnapshot(...).monitoring`; keep B2 metadata private, non-persisted, non-global, and separate from snapshot
- Monitoring/Praćenje remains product-critical and long-term belongs in Pregled and Kalendar, but Pregled/Kalendar monitoring integration is deferred until Plant detail behavior is proven and timing/display semantics are owner-approved. Observation capture, Dnevnik evidence, and stage confirmation are later Slice 8 steps; stage confirmation depends on owner-approved minimal stage vocabulary
...
- Runtime Slice 8 has not started; implementation must follow the approved Plant detail-only first step and refresh source-of-truth consistency before coding
