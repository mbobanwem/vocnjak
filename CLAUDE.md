...
- Runtime Slice 8 Step 4a documentation clarification is complete: the next allowed runtime direction is minimal Plant-detail-only, one-plant, free-standing `kind = "trap"` count capture using a bounded `trap_capture_sources[]` source map for the eight owner-approved trap rows only. Step 4a remains documentation-only here and does not implement runtime, UI, storage, validator, Pregled/Kalendar, program attachment, broad target/pest registry, symptom registry, stage vocabulary, diagnosis, treatment advice, pressure/severity scoring, or compliance behavior.
- Structured scouting/symptom/stage capture beyond Step 4a, program attachment, Dnevnik monitoring evidence beyond free-standing trap/note history, observation correction, and stage confirmation are later Slice 8 steps; stage confirmation depends on owner-approved minimal stage vocabulary.
...
- Runtime Slice 8 Step 1, Step 2, Step 3, and Step 4a documentation clarification are complete; later S8 runtime work must remain owner-approved and must preserve the Plant detail free-standing Observation boundary unless the owner explicitly opens the next step
