...
- B2 metadata-only projection boundary is complete. B2 resolves the owner-accepted 41-entry source working set into 36 projected B2 items with 5 merge groups and stable tracks: `monitoring_track` and `risk_awareness_track`. This grouping remains private/read-only audit/projection metadata in `index.html`, separate from `buildSeasonalSnapshot`, not persisted, not exposed as projection metadata globally, not a new user data model, and not a registry. Runtime Slice 8 Step 1 is complete: Plant detail consumes B2 metadata through a narrow S8 preview bridge and renders calm read-only monitoring/risk preview sections only for matching plant species. B2 still does not add `monitoring_programs[]`, `stage_vocabulary[]`, `awareness_definitions[]`, `target_registry[]`, `symptom_registry[]`, Pregled/Kalendar monitoring UI, or broader Slice 8 behavior.
- S8 Step 1 guardrails preserved: Pregled and Kalendar remain untouched; no observation writes, Dnevnik observation rows, routes, tap-through detail, or CTAs such as `Dodaj opažanje` were added; `Bez zapisa` is still absent from the first preview; `buildSeasonalSnapshot(...).monitoring` remains empty; B2 metadata stays private, non-persisted, non-global, and separate from snapshot
- S8 Step 2 note Observation capture is complete: Plant detail has `Dodaj opažanje`, saves one-plant free-standing `kind = "note"` records with `payload.text`, and renders them as factual Dnevnik evidence under `Opažanja` / plant history. Full Runtime Slice 8 is still not complete. Hard guardrails remain: no Pregled/Kalendar monitoring or risk UI, no structured trap/scouting/symptom/stage capture, no program attachment, no registries/vocabularies, no observation correction, no `Bez zapisa` / `Zadnji zapis`, no diagnosis/treatment, and no snapshot changes.
- Monitoring/Praćenje remains product-critical and long-term belongs in Pregled and Kalendar, but Pregled/Kalendar monitoring integration remains deferred until timing/display semantics are owner-approved. Structured observation capture, Dnevnik monitoring evidence beyond free-standing notes, and stage confirmation are later Slice 8 steps; stage confirmation depends on owner-approved minimal stage vocabulary. Next work is the next owner-approved S8 implementation prompt or S8 planning.
...
- Runtime Slice 8 Step 1 and Step 2 are complete; later S8 work must remain owner-approved and must preserve the Plant detail note-only boundary unless the owner explicitly opens the next step
