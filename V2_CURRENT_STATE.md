...
Phase: Runtime implementation continues. Runtime Slice 0, Slice 1, Slice 2, Slice 3, Slice 4, Slice 5, Slice 6, and Runtime Slice 7 through S7.4 are complete. Focused S3/S4 adversarial review after the post-Slice-4 safety fix passed and owner verification accepted the PASS. Pre-Slice-5 Add Plant date-validation message polish is complete. Pre-Slice-5 Action Window Seed prerequisite is complete (`df6a7fc Implement Action Window Seed prerequisite`); owner browser verification passed and focused adversarial review passed. Plan-template projection hardening is complete (`bcaf3a2 Harden plan-template projection rules`). Runtime Slice 5 is complete (`8bc630a Implement Runtime Slice 5 activity capture`). Runtime Slice 6 is complete (`99e76c8 Implement Runtime Slice 6 snapshot and calendar surfaces`). Pre-Slice-7 Action Window Notes Projection prerequisite is complete (`ad9a113 Project action-window notes into canonical catalog` and `a1b5307 Clean B1 action-window notes boundary`). Runtime Slice 7 is complete through S7.4 (`d61cc90 Harden S7 seasonal action detail display`). B2 metadata-only projection boundary is complete. Runtime Slice 8 Step 1 is complete. This patch records the approved S8 Step 2 note Observation shape; S8 Step 2 runtime implementation has not started.

Current goal:

Post-S8 Step 1. B2 metadata-only projection boundary is complete after S7.4. Runtime Slice 8 Step 1 added Plant detail read-only B2 monitoring/risk preview only. The approved S8 Step 2 note Observation shape is documented as `kind = "note"` with `payload.text`; runtime capture is not implemented.

Current immediate next step:

```text
Runtime Slice 8 Step 1 is complete. Plant detail now consumes the private B2 source-map projection boundary through an explicit S8 read path and renders calm read-only monitoring/risk preview sections only for matching plant species. Monitoring and risk-awareness are separated on Plant detail. Step 1 did not touch Pregled or Kalendar, add observation writes, add Dnevnik observation rows, add routes/CTAs, show `Bez zapisa`, or change `buildSeasonalSnapshot(...).monitoring`. B2 metadata remains private, non-global, non-persisted, and separate from snapshot. This documentation patch records the approved S8 Step 2 note Observation shape, but S8 Step 2 implementation has not started. Next implementation, if approved later, is Plant detail free-standing `kind = "note"` capture + Dnevnik evidence. Deferred: Pregled/Kalendar integration until timing/display semantics are owner-approved, structured trap/scouting/symptom/stage capture until registries/vocabularies are owner-approved, observation correction, stage confirmation, `Bez zapisa`, and `Zadnji zapis`. `V2_UX_MODEL.md` §16.7 outside-period disclosure remains deferred and requires explicit owner approval.
```
...
- Runtime Slice 8 Step 1 consumes B2 metadata only for Plant detail read-only monitoring/risk preview through a private S8 read path
...
Runtime Slice 7 is complete through S7.4. It did not rely on monitoring or awareness content. B2 metadata-only projection boundary is complete; Runtime Slice 8 Step 1 is the first runtime consumer of that metadata and is limited to Plant detail read-only preview.
