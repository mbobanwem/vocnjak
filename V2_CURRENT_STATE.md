...
```text
A2 — Default V2 / remove the #v2 gate is complete. A1 — archive/lifecycle baseline is complete using only archived_at / archive_reason / archive_note as the Plant archive fields. The next implementation session should be Plan Templates runtime fidelity / content parity after owner approval. Do not implement UX polish or any future/parked item until the owner explicitly opens that session.
```
