...
5. UX/design polish session. Phase A — Functional UX polish runtime is complete at `cc22d24 Polish V2 UX copy and Pregled click affordance`; owner mobile browser verification passed. The next gate is Phase B — V2 visual usability refresh planning/challenge for design tokens, typography, branded V2 header, status chips, cards/lists refresh, and forms/empty-state polish; Phase B requires separate per-session owner approval before any code lands, is `index.html`-only when later opened, and excludes any framework, build pipeline, `manifest.json` / `sw.js`, schema/model/storage/validator/import-export, Plan Templates content, or BBCH/phenology/regional-offset/urgency/diagnosis/treatment/AI work. Phase A does not close Phase B.
