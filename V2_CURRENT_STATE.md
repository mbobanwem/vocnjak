...
```text
A2 — Default V2 / remove the #v2 gate is complete. A1 — archive/lifecycle baseline is complete using only archived_at / archive_reason / archive_note as the Plant archive fields. Plan Templates runtime fidelity / content parity runtime patch is complete at c9645c4 Implement Plan Templates runtime parity fixes; owner browser verification passed. The next implementation session should be UX/design polish after explicit owner approval; read POLISH_BACKLOG.md first, especially the Claude Design section near the bottom and the Plan Templates runtime parity — UX/copy follow-ups section. Do not implement V2 Done audit, broader scouting, symptom registry, Observation.symptom, program-attached observations, BBCH/phenology engine, regional offsets, automatic date shifting, treatment recommendation, diagnosis, AI, paid/subscription work, or any other parked item until the owner explicitly opens that session.
```
...
5. UX/design polish session. Read `POLISH_BACKLOG.md` first, especially the Claude Design section near the bottom and the Plan Templates runtime parity — UX/copy follow-ups section; translate only approved design logic into bounded presentation polish after explicit owner approval.
