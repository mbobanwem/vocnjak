...
- Status: coverage gate context recorded; Step 7 numeric-band runtime, Step 7b context-only trap advisory runtime, and Step 7c bounded visual scouting runtime are pushed on main.
- Purpose (dual):
...
- This addendum does not close Slice 8. Step 6 runtime is complete for multi-plant free-standing `note` and `stage_obs` Observation capture only. Step 7c runtime is complete for bounded Plan Templates-backed visual scouting capture only.
...
- broad/general scouting registry, symptom registry, program-attached observation capture, observation correction, broader stage confirmation, and monitoring rows beyond the bounded Step 4a trap path and bounded Step 7c visual scouting path remain deferred to later owner-approved Slice 8 steps
- no `awareness_definitions[]`, broad `target_registry[]`, or `symptom_registry[]` in the current B2 projection, S8 Step 4a runtime source map, or S8 Step 7c runtime source map
