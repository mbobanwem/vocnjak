...
| plum | plum moth spring/summer | numeric-band `Klopke` runtime implemented in `905af41` for the trap part; source rows also contain fruit-signal scouting/symptom content | leave current trap runtime unchanged; route fruit-signal content to future structured Observation capture when Step 6 defines the target surface |
