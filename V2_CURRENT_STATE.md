...
Step 7 minimal runtime (`905af41 Implement S8 Step 7 trap advisory display`, local-only, not pushed) implements advisory display only for four source-backed S8-blocker rows:
...
This does not mean other trap rows are rejected or permanently unsupported. Step 7b exists to prevent silent loss of plan-template advisory guidance and to keep apple codling moth and other source-backed candidates visible on the roadmap rather than dropped.
...
- explicit owner decision on how to resolve each non-implemented row (source-check, implement, mark intentionally no numeric advisory, or move to a future owner-approved session).
