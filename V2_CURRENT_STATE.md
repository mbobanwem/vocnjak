...
Historical immediate-next-step note, superseded by the Post-UXR runtime stabilization record below:

```text
A2 — Default V2 / remove the #v2 gate is complete. A1 — archive/lifecycle baseline is complete using only archived_at / archive_reason / archive_note as the Plant archive fields. Plan Templates runtime fidelity / content parity runtime patch is complete at c9645c4 Implement Plan Templates runtime parity fixes. Phase A UX/copy polish runtime is complete at cc22d24 Polish V2 UX copy and Pregled click affordance; owner mobile browser verification passed. Phase B / UXR remains active: UXR.0 audit is complete at 5ef720d, UXR.1a V2-scoped design tokens are complete at 8163d1c, UXR.1b Fraunces + DM Sans typography loading is complete at 8702836, UXR.2a default route flip is complete at 559012c, UXR.2b app shell (top app bar + four-tab bottom nav + active tab state + #v2Content wrapper) is complete at 927555d Add Phase B V2 app shell, UXR.2c Postavke sheet + scaffold relocation + inline import confirm is complete at a78840a Add Phase B settings sheet, and UXR.2d Detail/form shell (top-bar text-back + hide bottom nav and Postavke on form routes) is complete at 17e5439 Refine Phase B detail and form shell. UXR.3a — Pregled hero + section order is complete at 44eff72 Add Phase B Pregled hero. UXR.3b — Cards/chips/list visual system is complete at 9c46712 Refine Phase B Pregled cards. UXR.3b refined Pregled cards/chips/list visual system Pregled-only via a new BEGIN UXR.3b CSS block in index.html, a consistent `.v2-seasonal-card-top` wrapper on Pregled `appendSeasonalCard` outputs gated on context current/upcoming/missed, three calm chip variants (Aktualno sea-pale, Pri kraju warm peach, Uskoro faded pale-sea) consuming UXR.1a tokens, a new `.v2-pregled-monitoring-card` neutral sea-pale info-card surface wrapping the existing B2 sentence + `Detalji su na kartonu voćke.` deflection note in appendB2PregledSection, Mlade voćke cards visually unified through Pregled-scoped CSS only, and tightened section rhythm. appendYoungTreeContextCard, appendPlantSeasonalActionCard, appendB2CalendarCard, appendSeasonalSection, renderKalendar, renderSeasonalActionDetail, renderDetail, and the UXR.3a hero are unchanged. Plant detail / Kalendar / Seasonal action detail / Dnevnik DOM are byte-identical to today. No route/storage/validator/import-export/Plan Templates/manifest/sw change. UXR.3c — Biljke Add Plant pattern / primary actions is complete at 166f26f Refine Phase B Biljke actions. UXR.3d species identity (Biljke row species icons + Plant detail species banner) is complete at `473e1c7 Add Phase B species identity`. The next session is UXR.4 — Forms + capture flows polish + Postavke label rename. Do not implement UXR.3d, UXR.4, UXR.5, V2 Done audit, broader scouting, symptom registry, Observation.symptom, program-attached observations, BBCH/phenology engine, regional offsets, automatic date shifting, treatment recommendation, diagnosis, AI, paid/subscription work, or any other parked item until the owner explicitly opens that session. Springcrest peach harvest vs bird-net timing remains unresolved as a future Plan Templates parity / content timing item, not UX polish.
```
...
Known follow-ups remain follow-ups, not blockers: hero month-chip contrast, corrected-marker contrast, the Springcrest peach harvest vs bird-net Plan Templates timing/content issue, any remaining focus-trap note if still open, and the final V2 Done audit.

### V2 Done path after Post-S8 Observation correction

Current controlling order is recorded in `V2_EXECUTION_ROADMAP.md §0`.
...
5. UX/design polish session. Phase A — Functional UX polish runtime is complete at `cc22d24 Polish V2 UX copy and Pregled click affordance`; owner mobile browser verification passed. Phase B — V2 visual usability refresh remains active. UXR.0 audit, UXR.1a V2-scoped design tokens, UXR.1b Fraunces + DM Sans typography loading, UXR.2a default route flip, UXR.2b app shell (top app bar + four-tab bottom nav + active tab state + `#v2Content` wrapper) at `927555d Add Phase B V2 app shell`, UXR.2c Postavke sheet + scaffold relocation + inline import confirm at `a78840a Add Phase B settings sheet`, and UXR.2d Detail/form shell (top-bar text-back + hide bottom nav and Postavke on form routes) at `17e5439 Refine Phase B detail and form shell` are complete, UXR.3a Pregled hero + section order is complete at `44eff72 Add Phase B Pregled hero`, and UXR.3b Cards/chips/list visual system is complete at `9c46712 Refine Phase B Pregled cards`; UXR.3c Biljke Add Plant pattern / primary actions is complete at `166f26f Refine Phase B Biljke actions`; UXR.3d species identity is complete at `473e1c7`; UXR.4 Forms + capture flows polish + Postavke label rename is complete at `9223f81 Refine Phase B form flows` (sticky bottom action bar on `add`/`activity_add`/`correction` form routes only, CSS-only scoped to `html.v2-active.v2-route-form .v2-plants-actions`; 15 user-facing `v2BackupStatus` `Slice 3 —` strings rewritten to clean Croatian with behavior unchanged; Postavke button labels and `ne znam` nesting were already complete from earlier sessions; Plant detail inline capture forms untouched and deferred to UXR.5b; no top-bar `Spremi`); next is `UXR.5 — Kalendar timeline (5a) / Plant detail rhythm (5b) / Dnevnik timeline (5c)`. Phase B requires separate per-session owner approval before any code lands, is `index.html`-only when later opened, and excludes any framework, build pipeline, `manifest.json` / `sw.js`, schema/model/storage/validator/import-export, Plan Templates content, or BBCH/phenology/regional-offset/urgency/diagnosis/treatment/AI work. Phase A does not close Phase B.
...
Not blocking V2 Done unless the owner explicitly reopens them:
