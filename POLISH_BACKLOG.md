...
Provenance note: this file is a parking lot for future polish, UX, UXR, and visual-design work. Entries may come from V1/legacy app observations, current review observations, or Claude Design recommendations for future V2 design sessions. They may describe old, current, placeholder, proposed, or future surfaces; they do not prove that equivalent V2 runtime UI currently exists and do not authorize implementation outside the current roadmap/session. Future UXR/design recommendations must wait for dedicated UXR/visual refresh sessions. B2/S8 monitoring and risk behavior remains governed by `V2_UX_MODEL.md` §0, `V2_DOMAIN_MODEL.md`, `CURRENT_STATE.md`, and `archive/v2/V2_EXECUTION_ROADMAP.md`.
...
- Fuji harvest timing — DONE (Phase 1 S6): owner-approved correction to 10.10.–05.11., band `late` (was 25.09.–15.10.). Source-backed by continental-HR ripening (late Oct–early Nov). Updated `V2_PLANT_CATALOG.md` (variety table + JSON) and `index.html` (runtime `speciesCatalog` harvestWindow + apple harvest `Napomene` example/Fuji line); maturity-first wording preserved; verifier PASS. No schema/validator/import-export/`window_def_id`/`action_type` change.
- Fantasia nectarine harvest timing — DONE (Phase 1 S6): owner-approved correction to 10.08.–05.09., band `mid` → `late` (was 01.08.–20.08.). Source-backed by continental-HR late-ripening (mid–late Aug into early Sep). Updated `V2_PLANT_CATALOG.md` (variety table + JSON) and `index.html` (runtime `speciesCatalog` harvestWindow + timing band); maturity-first wording preserved; verifier PASS. No schema/validator/import-export/`window_def_id`/`action_type` change.
...
Status: S5-A complete at `bcecdf0` — quince post-bloom fungicide projection, almond post-bloom fungicide projection, olive post-harvest pruning (Dec runtime + January note prose SAFE_TRANSFORM); verifier PASS after S5-A. S5-B complete — hazelnut pollination awareness and adult walnut summer pruning source-check flags resolved in `V2_ORCHARD_PLAN_TEMPLATES.md` (docs-only; no runtime, catalog, or window change; Fuji/Fantasia unchanged from S6).

S6 — Catalog timing decision batch
Goal: Fuji/Fantasia harvest timing source check and owner/S3 catalog decision.
Status: DONE — owner-approved windows projected to catalog + runtime: Fuji 10.10.–05.11. (`late`), Fantasia 10.08.–05.09. (`late`); committed "Correct Fuji and Fantasia harvest windows"; verifier PASS.
