...
Current Phase B session authority is `Claude-design/UXR_FINAL_PLAN.md` as local-only reference material. The older high-level outline below remains historical framing only; do not use it to override the accepted UXR.0/UXR.1a/UXR.1b/UXR.2a/UXR.2b/UXR.2c/UXR.2d session sequence.
...
Continue only through the currently opened UXR session. The next implementation session is UXR.2d — Detail / form behavior: back chevron + hide bottom nav on forms.
