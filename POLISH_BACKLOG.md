...
- Remaining follow-ups preserved here, not fixed by the stabilization pass: hero month-chip contrast, corrected-marker contrast, Springcrest peach harvest vs bird-net Plan Templates timing/content issue, final V2 Done audit, and any remaining focus-trap note if still open.
...
- corrected-marker (`ispravljeno`) contrast: brand color on cream is ~4.4; a one-line `--v2-brand-deep` fix is available later. Low-risk.
