...
Session 21 / STORE-W1 repository-side implementation is complete as of 2026-07-09. Scope stayed inside bridge/dev correctness and store-prereq stubs: `sw.js` now has a versioned STORE-W1 cache, local app/font/privacy/support assets, network-first navigation that preserves the Safari redirect rationale, and cached app-shell fallback when the origin is unreachable; runtime font loading is self-hosted through local WOFF2 files under `fonts/`; startup requests `navigator.storage.persist()` when available and treats unsupported/denied/failure outcomes as non-fatal; `manifest.json` stale description copy was cleaned without icon or identity redesign; minimal `privacy.html` and `support.html` stubs were added; and D2 bridge-channel evidence was captured without implementing pre-import backup or changing export re-persist policy. Verification: `node tools/verify-reg-r1-runtime.mjs` passed with `RESULT: PASS (32/32)`, `node tools/verify-content-parity.mjs` passed, `node --check sw.js` passed, manifest JSON parse passed, remote font reference scan found no `fonts.googleapis`/`fonts.gstatic` runtime dependency, Chrome/CDP proved all STORE-W1 cache assets present with only `vocnjak-store-w1-v2` active, server-down navigation with HTTP cache disabled loaded V2 through the service worker and validated `vocnjak_v2`, storage persistence denial in headless Chrome was non-fatal, invalid V2 import failed closed and preserved existing data, valid export/import round-trip preserved a one-plant V2 store and validated after import, and no V2 pre-import backup key was observed. Detailed evidence is in `STORE_W1_EVIDENCE.md`. Physical owner-iPhone Safari export/import evidence was not run in this environment; do not mark that device evidence PASS until the owner performs the recorded Safari steps.
...
Session 21 / CAP-SPIKE partial device evidence is recorded on 2026-07-07. The owner completed the real-iPhone proof on an iPhone 16 Pro running iOS 26.5, using Xcode 26.6 build 17F113, Apple ID Personal Team signing, Xcode-managed provisioning, a trusted development certificate, and the existing Capacitor scratch project outside the main repository. iOS real-device proof status: PASS for toolchain installation and operation, installed iOS platform support, physical iPhone pairing, Developer Mode, signing/provisioning, successful build/install/launch, main-tab navigation, plant creation, plant-list update, plant detail, seasonal content rendering, activity/evidence logging, journal persistence, calendar persistence, dashboard/overview rendering, full force-close/relaunch survival, created-plant persistence after relaunch, and logged-activity persistence after relaunch. Non-blocking real visual/native-shell finding: the top header/title overlaps the iOS status-bar / Dynamic Island safe-area region; this is evidence for future owning implementation/polish work and is not a CAP-SPIKE viability blocker. Android tooling is ready. The real-Android CAP-SPIKE proof is pending because no physical Android phone is currently available. We will resume this proof when an Android phone becomes available. Android emulator testing may be used later only as supplemental evidence and does not satisfy the locked real-Android acceptance criterion. Full CAP-SPIKE is NOT YET PASS because Android real-device evidence remains pending; non-dependent Session 21 work may continue by owner decision. This status record changes no runtime, native shell, store assets, storage migration, validators, import/export behavior, `index.html`, `manifest.json`, `sw.js`, catalog content, sync/cloud/accounts, notification work, or CAP-SPIKE acceptance criteria.
