...
8. **S8 Step 7b — Trap Advisory Source-Coverage Gap Map.** Required before Step 7 closure; documentation/tracking only.
   ...
   - Coverage map species and required next actions:
     - `apple` — codling moth / jabučni savijač: known gap; prior S3/S5 audit notes (`V2_S3_AUDIT_CONSOLIDATION.md`) say apple codling moth later gets the full trap + fruit-signal pattern; source row 337 references "lokalni pragovi" but does not list numeric bands. Required next action: source-check / extract source-backed bands or record explicit owner decision if no numeric bands are available; do not silently leave apple as "no advisory card forever".
     - `pear` — fruit moth / codling-like trap monitoring: needs monitoring split before advisory mapping; trap-vs-scouting ambiguity tracked at `V2_DOMAIN_MODEL.md` §3.2.1a:716. Required next action: resolve pear monitoring split; then source-check advisory bands.
     - `quince` — codling trap option: source-check required. Required next action: only add advisory if trap source-check resolves.
     - `peach` — Grapholita / fruit moth monitoring: future full pattern candidate; trap-vs-scouting ambiguity at source row 1228 tracked at `V2_DOMAIN_MODEL.md` §3.2.1a:716. Required next action: source-check bands + fruit-signal pattern.
     - `nectarine` — Grapholita / fruit moth monitoring: future full pattern candidate; trap-vs-scouting ambiguity at source row 1064 tracked at `V2_DOMAIN_MODEL.md` §3.2.1a:716. Required next action: source-check bands + fruit-signal pattern.
     - `walnut` — walnut fly / codling monitoring (source rows 2949, 2977): source-check required; current minimal Step 7 intentionally excludes both walnut rows because source carries qualitative wording only and adjoining template wording crosses the Step 7 treatment-framing hard boundary. Required next action: source-check before advisory mapping; if no numeric bands exist, record explicit owner decision.
     - `olive` — olive fly trap (source row 2455): no numeric bands in minimal source; context-only candidate; adjoining template wording for treatment is out of Step 7 scope. Required next action: keep as polish/context note unless source-backed bands are added; do not promote treatment-framed prose into advisory copy.
     - `sweet_cherry` — cherry fly: minimal bands implemented in `905af41`; fruit-signal / inspection-orientation polish still open. Required next action: optional future polish only.
     - `sour_cherry` — cherry fly: minimal bands implemented in `905af41`; fruit-signal / inspection-orientation polish still open. Required next action: optional future polish only.
     - `plum` — plum moth spring/summer: minimal bands implemented in `905af41`; "nagli skok s niske razine" comparative previous-week logic, fruit-signal pattern, and harvest-proximity caveat polish still open. Required next action: optional future polish / source-backed extension.
   - Hard rules (binding for any future Step 7 expansion): do not invent trap thresholds; do not create generic global bands; do not turn advisory text into treatment instruction (no product names, no dosage, no `prskaj sada`, no `tretiraj`, no diagnosis, no `kasniš`/`obavezno`/`due`/`overdue`/`missed`, no checkbox/task framing, no compliance UX); do not persist advisory `band` / `level` / `severity` / `pressure_score` / `advisory` in Observation payload; if a species lacks source-backed numeric bands, record it as a source gap or owner-decision item — do not silently omit it; apple codling moth must remain visible in the gap map until resolved.
   - Step 7 closure requires: this gap map recorded; owner review of each row's current status and required next action; and explicit owner decision on how to resolve each non-implemented row (source-check, implement, mark intentionally no numeric advisory, or move to a future owner-approved session).
