...
2. Step 7e has completed read-only B2 source-row guidance parity for S8 by extending only `B2_READONLY_GUIDANCE_BY_SOURCE_ROW`; the next S8 work is the S8 closure gate / tracker sync to record that Step 7 coverage is complete and decide/mark whether S8 can close.
...
- Next S8 work is the S8 closure gate / tracker sync to record that Step 7 coverage is complete and decide/mark whether S8 can close
