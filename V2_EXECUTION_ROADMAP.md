...
Phase B / UXR progress record: Phase B / UXR is active. `UXR.0 — Runtime IA + ID audit` is complete at `5ef720d Add Phase B UXR audit`. `UXR.1a — V2-scoped Phase B design tokens` is complete at `8163d1c Add Phase B V2 design tokens`; it changed only the `index.html` CSS token block under `.v2-active`, consumed no tokens, and changed no JS, DOM, routes, storage, validators, import/export behavior, Plan Templates, `manifest.json`, or `sw.js`. `Claude-design/` remains local reference material only and is not committed. Later UXR carry-forward notes: runtime uses `plant.species`, not `plant.species_id`, so UXR.3d must map species visuals from `plant.species`; Pregled section-order correction belongs to UXR.3a/UXR.3b, not UXR.1b; UXR.2a later flips empty hash to Pregled while keeping `#v2` as the Biljke compatibility alias. The next session is `UXR.1b — Fraunces + DM Sans typography link`.
...
- Phase B / UXR is active. UXR.0 audit is complete at `5ef720d`; UXR.1a V2-scoped Phase B design tokens are complete at `8163d1c`. UXR.1a changed only the `index.html` CSS token block under `.v2-active`; no JS, DOM, routes, storage, validators, import/export behavior, Plan Templates, `manifest.json`, or `sw.js` changed. The next session is UXR.1b — Fraunces + DM Sans typography link.
