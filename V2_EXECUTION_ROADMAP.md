...
**Status:** S11.A, S11.B, S11.C1, S11.C2, and S11.D complete. Runtime implementation is underway; Runtime Slices 0–7 are complete through S7.4. B2 metadata-only projection boundary is complete; Runtime Slice 8 Step 1, Step 2, Step 3, Step 4a, Step 5a, Step 6, and Step 7c are runtime-complete. Step 7c docs lock was committed at `7e388c5`; Step 7c runtime was committed at `588e413`. Full Runtime Slice 8 is not complete; broader phenology-aware stage confirmation, broad/general scouting registry, and symptom registry remain deferred until separately owner-approved.
...
7. **S8 Step 7 — Template runtime coverage gate + source-backed trap advisory bands + read-only observation status.** Coverage gate context recorded; Step 7 numeric-band runtime, Step 7b context-only trap advisory runtime, and Step 7c bounded visual scouting runtime are pushed on main.
...
8. **S8 Step 7b — Trap Advisory Source-Coverage Gap Map.** Recorded; documentation/tracking only.
   ...
   - After Step 7c: this gap map is recorded; visual-sign rows approved for Step 7c now have a bounded source-row-backed `Observation.kind = "scouting"` destination; trap capture/advisory remains separate from scouting capture; any further optional trap numeric extension, broad/general scouting registry, or symptom registry requires owner decision.
...
9. **S8 Year-1–2 young-tree relevance addendum.** **Runtime-complete; UX-spec sanctioned.** Does not close Slice 8.
   ...
   - Does not close Slice 8. Step 7c runtime is complete for bounded Plan Templates-backed visual scouting capture only.
   - Status note: runtime is pushed on main at `9074e7b Implement S8 year-1-2 young-tree relevance`.

S8 remaining steps:

Runtime Slice 8 is not closed. Steps 1, 2, 3, 4a, 5a, 6, and 7c are runtime-complete. Step 7 numeric-band runtime is pushed on main at `905af41` for sweet cherry, sour cherry, and plum rows 654/860/1596/1643. Step 7b context-only trap advisory runtime is pushed on main at `b1d840c` for apple row 337, olive row 2455, walnut row 2949, and walnut row 2977. Step 7c bounded visual scouting runtime is pushed on main at `588e413` after the docs lock at `7e388c5`.
...
4. Multi-plant grouping for free-standing `note` / `stage_obs` Observations is complete in Step 6; bounded multi-plant visual scouting capture is complete in Step 7c; future structured scouting/symptom Observation grouping beyond Step 7c still requires owner approval.
...
- Next S8 work beyond Step 7c requires explicit owner approval and must preserve the deferred return map
