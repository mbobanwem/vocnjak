...
8. **S8 Step 7b — Trap Advisory Source-Coverage Gap Map.** Required before Step 7 closure; documentation/tracking only.
   ...
   - Coverage map species and required next actions:
     ...
     - `plum` — plum moth spring/summer: numeric-band `Klopke` runtime implemented in `905af41` for the trap part; source rows also contain fruit-signal scouting/symptom content. Required next action: leave current trap runtime unchanged; route fruit-signal content to future structured Observation capture when Step 6 defines the target surface.
   ...
   - Step 7 closure requires: this gap map recorded; owner review of each row's current status and required next action; explicit owner-reviewed source/destination decision for each remaining row; and Step 6 impact clear for structured scouting/symptom Observation capture.
...
Runtime Slice 8 is not closed. Steps 1, 2, 3, 4a, and 5a are runtime-complete (items 1–5 above). Step 7 numeric-band runtime is pushed on main at `905af41` for sweet cherry, sour cherry, and plum rows 654/860/1596/1643. Step 7b context-only trap advisory runtime is pushed on main at `b1d840c` for apple row 337, olive row 2455, walnut row 2949, and walnut row 2977. Two remaining items gate Slice 8 closure:
