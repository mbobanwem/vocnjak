...
Status: STEPS 1–6 RUNTIME COMPLETE; STEP 7 NUMERIC-BAND RUNTIME AND STEP 7B CONTEXT-ONLY RUNTIME PUSHED ON MAIN; STEP 7B OWNER-REVIEWED SOURCE/DESTINATION DECISIONS OPEN; STEP 7 NOT FULLY CLOSED — Plant detail read-only B2 preview, Plant detail free-standing note Observation capture + Dnevnik evidence, read-only Pregled/Kalendar B2 monitoring/risk visibility, bounded free-standing trap count capture, minimal free-standing stage diary `kind = "stage_obs"` capture, and multi-plant free-standing `note` / `stage_obs` Observation capture are implemented in `index.html`. Step 7 numeric-band runtime covers sweet cherry, sour cherry, and plum rows 654/860/1596/1643. Step 7b context-only trap advisory runtime at `b1d840c` also covers apple row 337, olive row 2455, walnut row 2949, and walnut row 2977. Step 6 runtime is complete at `8c7d135 Implement multi-plant observation capture` for multi-plant free-standing `note` and `stage_obs` capture only. Runtime Slice 8 is not closed; Step 7 is not fully closed until remaining row dispositions are documented and owner-reviewed.
...
8. **S8 Step 7b — Trap Advisory Source-Coverage Gap Map.** Required before Step 7 closure; documentation/tracking only.
   ...
   - Coverage map species and required next actions:
     ...
     - `pear` — row 516 fruit moth / codling-like monitoring: Row 516 pear is split-required: trap Observation candidate with source-backed numeric bands plus fruit-signal scouting/symptom Observation content. It is not Step 7b context-only. Required next action: add any `Klopke`/advisory display only for the trap part after owner-approved split; route fruit signals to future structured Observation capture.
     - `quince` — row 2004 codling-like fruit scouting: Row 2004 quince is not a trap advisory candidate in the current source. It is source-backed visual scouting/symptom Observation content; the source explicitly says pheromone trap use is not described until confirmed by sources. Do not show `Klopke`, trap count, or invented thresholds for quince. Quince remains present in `V2_PLANT_CATALOG.md`; this is not catalog deferral. Required next action: route visible fruit signs to future structured scouting/symptom Observation capture.
     - `peach` — row 1228 Grapholita / fruit moth monitoring: Row 1228 peach is mixed optional-trap plus visual shoot/fruit-sign content, structurally same as nectarine row 1064. It is not a clean Step 7b `Klopke` row and has no source-backed numeric bands. Required next action: route visible signs to future structured scouting/symptom Observation capture; decide separately whether an optional trap capture path is source-backed enough.
     ...
     - `plum` — plum moth spring/summer: numeric-band `Klopke` runtime implemented in `905af41` for the trap part; source rows also contain fruit-signal scouting/symptom content. Required next action: leave current trap runtime unchanged; route fruit-signal content to future structured scouting/symptom Observation capture after owner approval.
   ...
   - Step 7 closure requires: this gap map recorded; owner review of each row's current status and required next action; explicit owner-reviewed source/destination decision for each remaining row; and any remaining structured scouting/symptom destination decisions owner-reviewed.
...
9. **S8 Year-1–2 young-tree relevance addendum.** **Runtime-complete; UX-spec sanctioned.** Does not close Slice 8.
   ...
   - Does not close Slice 8. Step 7b remains open.
   - Status note: runtime is pushed on main at `9074e7b Implement S8 year-1-2 young-tree relevance`.

S8 remaining steps:

Runtime Slice 8 is not closed. Steps 1, 2, 3, 4a, 5a, and 6 are runtime-complete (items 1–6 above). Step 7 numeric-band runtime is pushed on main at `905af41` for sweet cherry, sour cherry, and plum rows 654/860/1596/1643. Step 7b context-only trap advisory runtime is pushed on main at `b1d840c` for apple row 337, olive row 2455, walnut row 2949, and walnut row 2977. One remaining item gates Slice 8 closure:

- S8 Step 7b — Trap Advisory Source-Coverage Gap Map owner-reviewed source/destination decisions; Step 7 is not fully closed until remaining row dispositions are documented and owner-reviewed.
...
4. Multi-plant grouping for free-standing `note` / `stage_obs` Observations is complete in Step 6; stabilize identifiers before future structured scouting/symptom Observation grouping.
...
- Later only: program-attached observation capture, structured scouting/symptom capture, monitoring-context record displays, and broader phenology-aware stage confirmation.
...
- Still deferred: structured scouting/symptom/stage capture, program-attached observations, observation correction, monitoring-context record-status copy, diagnosis/treatment logic, and stage confirmation
- Next S8 work beyond Step 4a requires explicit owner approval and must preserve the deferred return map
