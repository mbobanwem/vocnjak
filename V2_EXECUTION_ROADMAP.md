...
Post-S8 Observation correction package order:

1. Documentation lock for Observation correction using the existing additive Correction model.
2. Runtime validator/model implementation for supported Observation kinds only: `note`, `trap`, `stage_obs`, and `scouting`.
3. UI/display implementation after validator/model behavior is strict and effective composition is deterministic.
4. Tracker sync after runtime is verified.

Do not split this package into artificial endless microsteps. Keep the order safe: docs lock first, validator/model second, UI/display third, tracker sync last.
...
2. Post-S8 Observation correction docs lock is recorded; the next owner-approved Observation correction work should be runtime validator/model implementation, then UI/display, then tracker sync.
...
- Post-S8 / owner-approved future work remains: observation correction; broader scouting capture beyond Step 7c; symptom capture / `Observation.symptom`; owner-approved symptom source map / registry before any `symptom_code`; program-attached observations; broader phenology-aware stage confirmation beyond Step 5a diary capture; low-risk fallback/display polish; AI-assisted image analysis or paid/subscription AI; and any diagnosis/treatment recommendation system only if separately owner-approved with strict guardrails
- Next work after the Post-S8 Observation correction docs lock should be owner-approved runtime validator/model implementation first, then UI/display, then tracker sync
